...
[![PyPI version](https://badge.fury.io/py/tap-postgres.svg)](https://badge.fury.io/py/tap-postgres)

[![CircleCI Build Status](https://circleci.com/gh/singer-io/tap-postgres.png)](https://circleci.com/gh/singer-io/tap-postgres)

[Singer](https://www.singer.io/) tap that extracts data from a [PostgreSQL](https://www.postgresql.org/) database and produces JSON-formatted data following the [Singer spec](https://github.com/singer-io/getting-started/blob/master/docs/SPEC.md).

```bash
$ python3 -m venv ~/singer/tap-postgres
$ source ~/singer/tap-postgres/bin/activate
$ pip3 install tap-postgres
$ tap-postgres --config config.json --discover
$ tap-postgres --config config.json --properties properties.json --state state.json
```
...
This section dives into basic usage of `tap-postgres` by walking through extracting
data from a table. It assumes that you can connect to and read from a PostgreSQL
database.
...
```bash
$ git clone git@github.com:singer-io/tap-postgres.git
$ cd tap-postgres
$ python3 -m venv .
$ python3 install .
```
...
There's some important business data siloed in this PostgreSQL database -- we need to
extract it. Here's the table we'd like to sync:

```
postgres> select * from example_db.animals;
+----|----------|----------------------+
...
These are the same basic configuration properties used by the PostgreSQL command-line
client (`psql`).
...
In sync mode, `tap-postgres` consumes the catalog and looks for tables and fields
...
In the above example, we invoked `tap-postgres` without providing a _state_ file
...
```
postgres> insert into animals (name, likes_getting_petted) values ('dog', true), ('elephant', true), ('frog', false);
```

```bash
$ tap-postgres -c config.json --properties properties.json --state state.json
```
...
Log based replication works in conjunction with a state file to only extract
new records each time the tap is invoked. This requires a replication key to be
specified in the table's metadata as well.

#### wal2json decoder plugin

Singer tap for PostgreSQL supporting Full Table & Logical Replication
using the [wal2json](https://github.com/eulerto/wal2json) decoder plugin.

```
SELECT * FROM pg_create_logical_replication_slot('stitch', 'wal2json');
```

- Issue 85: [Support for large transactions](https://github.com/singer-io/tap-postgres/issues/85)
- PR 91: [wal2json format v2](https://github.com/singer-io/tap-postgres/pull/91)

#### PostgreSQL replication config

For an example see the [CircleCI dev config](https://github.com/singer-io/tap-postgres/blob/master/.circleci/postgresql.conf)

  - PostgreSQL 9.6 docs: [19.6.1 Replication: Sending Server(s)](https://www.postgresql.org/docs/9.6/runtime-config-replication.html#GUC-MAX-WAL-SENDERS)
...
##### AWS RDS
  ...
  - `logical_replication` in parameter(reboot)= 1
  - Read more at [PostgreSQL on Amazon RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_PostgreSQL.html#PostgreSQL.Concepts.General.FeatureSupport.LogicalReplication)
  - For Aurora see [replication notes](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraPostgreSQL.Replication.Logical.html#AuroraPostgreSQL.Replication.Logical.Configure)
